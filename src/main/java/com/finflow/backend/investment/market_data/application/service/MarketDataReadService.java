...
/**
 * Centralized read gateway for market-data entities and read projections.
 * <p>
 * No @Transactional here by design; transaction boundary stays in UseCase layer.
 */
@Component
@RequiredArgsConstructor
public class MarketDataReadService {
    ...
    public List<Company> suggestBySymbolPrefix(String prefix, int limit) {
        String normalizedPrefix = Optional.ofNullable(prefix).orElse("").trim().toUpperCase();
        if (normalizedPrefix.isEmpty()) {
            return List.of();
        }
        return companyRepository.findByIdStartingWithIgnoreCaseOrderByIdAsc(
                normalizedPrefix,
                PageRequest.of(0, Math.max(1, limit))
        );
    }

    public List<Company> suggestByName(String keyword, int limit) {
        String normalizedKeyword = Optional.ofNullable(keyword).orElse("").trim();
        if (normalizedKeyword.isEmpty()) {
            return List.of();
        }
        return companyRepository.findByCompanyNameContainingIgnoreCaseOrderByIdAsc(
                normalizedKeyword,
                PageRequest.of(0, Math.max(1, limit))
        );
    }

    public List<Company> loadCompaniesBySymbolsUpper(List<String> symbolsUpper) {
    ...
    public List<IndustryNode> loadIndustryNodes() {
        ...
        rows.sort(
                Comparator.comparing(IndustryNode::getLevel, Comparator.nullsLast(Comparator.naturalOrder()))
                        .thenComparing(IndustryNode::getIcbCode, Comparator.nullsLast(Comparator.naturalOrder()))
                        .thenComparing(IndustryNode::getNameVi, Comparator.nullsLast(Comparator.naturalOrder()))
        );
        return rows;
    }
    ...
    public List<BankIncomeStatement> loadBankIncomesLastQuarters(String companyId, int limit) {
    ...
    public List<FinancialIndicator> loadFinancialIndicators(String companyId, Integer annualLimit, Integer quarterlyLimit) {
    ...
    public List<FinancialIndicator> loadFinancialIndicatorsForValuations(String companyId, Integer annualLimit) {
        if (annualLimit == null) {
            return financialIndicatorRepository.findByCompanyIdOrderByYearAscQuarterAsc(companyId);
        }
    ...
    public List<FinancialIndicator> loadFinancialIndicatorsForValuationsByRange(
            String companyId,
            LocalDate startDate,
            LocalDate endDate,
    ...
    public List<CompanyDividend> loadCompanyDividends(String companyId, Integer annualLimit) {
        if (annualLimit == null) {
    ...
    public List<BankBalanceSheet> loadBankBalances(String companyId, Integer annualLimit, Integer quarterlyLimit) {
        if (!isBoundedLoad(annualLimit, quarterlyLimit)) {
    ...
    public List<BankIncomeStatement> loadBankIncomes(String companyId, Integer annualLimit, Integer quarterlyLimit) {
        if (!isBoundedLoad(annualLimit, quarterlyLimit)) {
            return bankIncomeStatementRepository.findByCompanyIdOrderByYearAscQuarterAsc(companyId);
        }
        int cap = computeTimeSeriesCap(annualLimit, quarterlyLimit);
    ...
    public List<NonBankBalanceSheet> loadNonBankBalances(String companyId, Integer annualLimit, Integer quarterlyLimit) {
        if (!isBoundedLoad(annualLimit, quarterlyLimit)) {
    ...
    public List<NonBankIncomeStatement> loadNonBankIncomes(String companyId, Integer annualLimit, Integer quarterlyLimit) {
        if (!isBoundedLoad(annualLimit, quarterlyLimit)) {
            return nonBankIncomeStatementRepository.findByCompanyIdOrderByYearAscQuarterAsc(companyId);
        }
        int cap = computeTimeSeriesCap(annualLimit, quarterlyLimit);
    ...
    public List<NonBankIncomeStatement> loadNonBankIncomesLastQuarters(String companyId, int limit) {
        int cap = Math.max(1, limit);
        return new ArrayList<>(
                nonBankIncomeStatementRepository.findByCompanyIdOrderByYearDescQuarterDesc(companyId, PageRequest.of(0, cap))
        );
